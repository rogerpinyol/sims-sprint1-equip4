...
<div class="h-full min-h-screen flex flex-col" id="appBody">
  ...
  <button id="btnOpenSidebar" class="p-2 rounded-md bg-slate-200 hover:bg-slate-300 md:hidden" aria-label="Menu">
  ...
  <h1 class="text-lg font-semibold tracking-tight">EcoMotion</h1>
      ...
      <div class="flex items-center gap-2 text-sm flex-wrap justify-end">
        <span class="hidden sm:inline">Hello, <?= e($user['name']) ?></span>
    ...
    <aside id="sidebar" class="w-72 bg-white border-r border-[color:var(--brand-green-light)] z-40 transform -translate-x-full transition-transform duration-200 absolute inset-y-0 left-0 md:translate-x-0 md:relative md:flex md:flex-col shadow-lg md:shadow-none flex-shrink-0">
      <div class="flex flex-col h-full">
        <div class="flex items-center justify-between px-4 py-3 border-b border-[color:var(--brand-green-light)] bg-slate-50 shrink-0">
          ...
          <button id="btnCloseSidebar" class="p-1 rounded-md hover:bg-slate-200 md:hidden" aria-label="Close">
        ...
        <div class="flex-1 min-h-0 p-3 overflow-y-auto">
          ...
          <div id="vehiclesList" class="space-y-3 text-sm md:hidden">
            <div class="text-slate-500">Loading vehicles...</div>
          </div>
          <!-- Desktop table -->
          <div id="vehiclesTableWrap" class="hidden md:block overflow-x-auto">
            <div class="text-slate-500 text-sm">Loading vehicles...</div>
          </div>
        </div>
        <div class="p-3 border-t border-[color:var(--brand-green-light)] bg-slate-50 shrink-0">
          <div class="text-xs mb-2 text-slate-500">Filter by status</div>
          <form id="statusFilterForm" class="grid grid-cols-2 gap-2 text-xs">
            <label class="flex items-center gap-1"><input type="checkbox" name="status" value="available" class="accent-blue-600" checked> <span>Available</span></label>
            <label class="flex items-center gap-1"><input type="checkbox" name="status" value="booked" class="accent-yellow-500" checked> <span>Booked</span></label>
            <label class="flex items-center gap-1"><input type="checkbox" name="status" value="maintenance" class="accent-orange-500" checked> <span>Maintenance</span></label>
            <label class="flex items-center gap-1"><input type="checkbox" name="status" value="charging" class="accent-blue-500" checked> <span>Charging</span></label>
